...
export default function Header() {
    ...
    useEffect(() => {
        ...
        window.addEventListener('resize', () => { // Add an event listener for the 'resize' event
            init(); // Call the 'init' function
        });
        
        return () => {
            window.removeEventListener('resize', () => { // Remove the 'resize' event listener when the component is unmounted
                init(); // Call the 'init' function
            });
        };
    });
